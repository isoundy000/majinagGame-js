        ...
        if (this._currIndex === 0) {
            ...
            if (cc.vv.gameNetMgr.chupai_PK_last !== undefined) {
                if (cc.vv.gameNetMgr.chupai_PK_last[0] === 54 && cc.vv.gameNetMgr.chupai_PK_last.length === 1) {
                    ...
                    this._activeGuoBtn.children[0].getComponent(cc.Button).interactable = false
                } else {

                    this._activeGuoBtn.getComponent(cc.Button).interactable = true
                    this._activeGuoBtn.children[0].getComponent(cc.Button).interactable = true
                }
            }
    ...
    initOtherHolds: function (index, holds) {
        ...
        if (index !== 0) {
            ...
            for (let i = 0; i < hold.length; i++) {
                ...
                if (index === 1) {
                    card.rotation = 270;
                    card.y = i * 0.5 * height * 0.5 * 0.6 - (hold.length - 1) * 0.5 * height * 0.5 * 0.3;
                } else {
                    card.rotation = 90;
                    card.y = (hold.length - 1) * 0.5 * height * 0.5 * 0.3 - i * 0.5 * height * 0.5 * 0.6  - 0.5 * height * 0.5 * 0.8;
