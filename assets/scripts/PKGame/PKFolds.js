cc.Class({
    ...
    initFolds: function (seatData) {
        ...
        for (var i = 0; i < folds_length; ++i) {
            ...
            if (cc.vv.PKReplayMgr.isReplay()) {
                ...
                if (localIndex !== 0)
                    this.centerVertical(sprite, folds_length, this._holdspai[localIndex - 1], i);
                this.setSpriteFrameByMJID("default-", sprite, mjid);
            } else {
                if (localIndex !== 0) {
                    let holdspk = cc.vv.prefabMgr.getPrefab('prefabs/PKGame/MyPK');
                    let card = cc.instantiate(holdspk);
                    let sprite = card.getComponent(cc.Sprite);
                    sprite.node.active = true;
                    sprite.node.mjId = mjid;
                    cc.vv.controlMgr.setSpriteFrameByMJID('default-', sprite, mjid);
                    card.parent = this._foldspai[localIndex - 1];
                    card.scale = 0.6;
                    let height = card.width;
                    if(localIndex===1) {
                        card.x = (folds_length - 1) * 0.5 * height * 0.6 * 0.3 - height * 0.8 * 0.3 * (folds_length - i) + 0.5 * height * 0.6 * 0.3;
                    }else{
                        card.x = (folds_length - 1) * 0.5 * height * 0.6 * 0.3 - height * 0.8 * 0.3 * (folds_length - i) + 0.5 * height;
                    }
    ...
    centerVertical: function (sprite, len, holds, index) {
        var height = sprite.node.height * 0.5;
        var starth = (this.node.height - 209 * 0.88 - (len - 1) * height * 0.3 - height) * 0.5;
        var posy = holds.convertToNodeSpaceAR(cc.p(0, starth + 230 * 0.88 + (index + 1) * height * 0.3));
        // ggg
