...
cc.Class({
    ...
    onLoad: function () {
		...
		this._others = this.node.getChildByName("JoinGame");//history.getChildByName('joinGame');
    },
    ...
    onBtnBackClicked: function() {
        ...
        if (this._curRoomInfo == null) {
            ...
            cc.vv.utils.showDialog(this._historyRoom, 'body', false, this._history);
        } else {
            this._historyRoom.active = true;

			// cc.vv.utils.showFrame(this._historyDetail, 'head', 'body', false);
            cc.vv.utils.showDialog(this._historyDetail, 'body', false);
    ...
    onBtnHistoryClicked: function() {
        ...
        cc.vv.utils.showDialog(this._historyRoom, 'body', true);
...
*/
        ...
        for (var i = 0; i < data.length; ++i) {
            ...
            if (cc.vv.userMgr.returnRoomId != null) {
               ...
               if (roomInfo.opType === 3){ //扑克为3，以后要去掉
                //node.getChildByName('btnReplay').getComponent(cc.Button).interactable = false;
                node.getChildByName('btnReplay').active = false;
                node.getChildByName('btnShare').active = false;
               } else{
                //node.getChildByName('btnReplay').getComponent(cc.Button).interactable = true;
                node.getChildByName('btnReplay').active = true;
                node.getChildByName('btnShare').active = true;
               }
            ...
            for (var j = 0; j < seats.childrenCount; j++) {
                ...
                } else { seat.getComponent(cc.Label).string = "" }
    ...
    getGameListOfRoom: function(idx) {
        ...
        cc.vv.userMgr.getGamesOfRoom(roomInfo.uuid, function(data) {
            if (data != null && data.length > 0) {
                ...
                cc.vv.utils.showDialog(self._historyDetail, 'body', true);
