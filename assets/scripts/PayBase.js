function PayBase(){
	...
	this.database='guoyao';
}
function md5(str){
	return cc.GMd5(str);
}
PayBase.prototype.get=function(path,data,callback){
...
PayBase.prototype.makeUrl=function(path,data){
	var str = "?";
	for(var k in data){
		if(str != "?"){
			str += "&";
...
PayBase.prototype.goodsList=function (callback){
...
PayBase.prototype.payUrl=function(uid,type){
	var data={
		gameid:uid,
		type:type,
...
PayBase.prototype.getKey=function(){
    return md5("db_game_"+this.database);
};

PayBase.prototype.sign=function(data,map){
    if(map==null){
