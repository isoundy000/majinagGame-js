...
var ACTION_TING = 8;
var ACTION_BUHUA = 9;
var ACTION_SHAOLONG = 10;
var ACTION_LAOLONG = 11;
var ACTION_LAOLAOLONG = 12;
var ACTION_KAN = 13;
var ACTION_LOUZI = 14;
...
cc.Class({
    ...
    takeAction:function(){
        ...
        if(this._lastAction != null && this._lastAction.type == ACTION_CHUPAI){
            if(action != null && action.type != ACTION_PENG && action.type != ACTION_GANG && action.type != ACTION_HU && action.type != ACTION_CHI && action.type != ACTION_ZIMO && action.type != ACTION_BUHUA && action.type != ACTION_SHAOLONG && action.type != ACTION_LAOLONG && action.type != ACTION_LAOLAOLONG && action.type != ACTION_KAN && action.type != ACTION_LOUZI && action.type != ACTION_QI && action.type != ACTION_ACTION_CHI && action.type != ACTION_ACTION_PENG && action.type != ACTION_ACTION_GANG && action.type != ACTION_ACTION_HU){
        ...
        else if(action.type == ACTION_SHAOLONG){
            cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 5});
            return 1.0;
        }
        else if(action.type == ACTION_LAOLONG){
            cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 10});
            return 1.0;
        }
        else if(action.type == ACTION_LAOLAOLONG){
            cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 15});
            return 1.0;
        }
        else if(action.type == ACTION_KAN){
            cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 1});
            return 1.0;
        }
        else if(action.type == ACTION_LOUZI){
            cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:true, cgType: 2});
            return 1.0;
        }
