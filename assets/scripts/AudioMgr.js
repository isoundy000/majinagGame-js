cc.Class({
    ...
    getMJGameUrl: function (url, isLanguage) {
        ...
        if (isLanguage == true) {
            ...
            var gameUrl = "resources/sounds/MJGame/" + getLName + "/" + getSName + "/";
        }
    ...
    getPKGameUrl: function (url,ispk) {
        if(ispk){

            var audioPath = cc.url.raw("resources/sounds/PKGame/" + url);
        }else{
            var audioPath = cc.url.raw("resources/sounds/MJGame/" + url);
        }
        return audioPath;
    },
    ...
    playMJGameBGM: function (url,isPK){
        
        if (isPK===false || isPK ===undefined) {
            var audioUrl = this.getPKGameUrl(url, false);
        } else var audioUrl = this.getPKGameUrl(url, true);
        console.log(audioUrl);
    ...
    playMJGameSFX: function (url,ispk){
        // if (isPK===false || isPK ===undefined) {
        //     var audioUrl = this.getMJGameUrl(url, true);
        // } else var audioUrl = this.getPKGameUrl(url);
        if(cc.vv.PKReplayMgr.isReplay()){
            var audioUrl = this.getMJGameUrl(url, true);
        }else{

            var audioUrl = this.getMJGameUrl(url, ispk);
        }
        console.log(audioUrl);
